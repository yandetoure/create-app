<x-dashboard-layout>
    <x-slot name="title">Modifier la Catégorie de Fonctionnalités</x-slot>
    ...
    <div class="max-w-4xl mx-auto py-12">
        <div
            ...
            <div class="flex items-center justify-between mb-12 relative">
                <h3 class="text-3xl font-black tracking-tighter">Edition : {{ $featureCategory->name }}</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-3">
                        <label class="text-[10px] font-black uppercase tracking-widest text-indigo-400 ml-2">Nom de la
                            catégorie</label>
                        <input type="text" name="name" value="{{ $featureCategory->name }}" required
                    ...
                    <div class="space-y-3">
                        <label
                            class="text-[10px] font-black uppercase tracking-widest text-indigo-400 ml-2">Icône</label>
                        <input type="text" name="icon" value="{{ $featureCategory->icon }}"
                            class="w-full bg-white/5 border border-indigo-600/20 rounded-2xl px-6 py-5 text-lg font-bold text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition outline-none">
                    </div>
                </div>
