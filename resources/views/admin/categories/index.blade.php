<x-dashboard-layout>
    <x-slot name="title">Gestion des Catégories</x-slot>
    ...
    <div class="space-y-12">
                            ...
                            <tr class="hover:bg-white/[0.03] transition group">
                                ...
                                <td class="px-8 py-6 text-right space-x-2">
                                    ...
                                    <form action="{{ route('admin.categories.destroy', $category) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div
            ...
            <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-8 relative">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="name" required
                            ...
                            placeholder="ex: Service de Streaming">
                    </div>
                    <div class="space-y-3">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2">Identifiant
                            de l'icône</label>
                        <input type="text" name="icon" required
                            class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder-gray-600 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition outline-none"
                            placeholder="Heroicon name (ex: cloud)">
                    </div>
                </div>
                <div class="space-y-3">
                    <label
                        class="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2">Description</label>
                    <textarea name="description" rows="3"
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder-gray-600 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition outline-none resize-none"
                        placeholder="Décrivez brièvement le type de projets dans cette catégorie..."></textarea>
                </div>
